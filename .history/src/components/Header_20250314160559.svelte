<script>
	...
	const menus = {
		...
		partners: [
			...
			{ name: 'Local-Regen-Net', path: '/ons-local-regen-net' },
			{ name: 'Woningcorporaties', path: '/woningcorporaties' },
			{ name: 'Investeerders', path: '/investeerders' },
			{ name: 'Gemeenten', path: '/gemeenten' },
			{ name: 'Contact', path: '/contact-zakelijk' }
		]
	};
...
<div class="w-full bg-black">
	<div class="max-w-[1400px] mx-auto">
		...
		<div class="flex text-white pt-4 px-6">
			<button 
				class="flex items-center gap-2 px-4 py-2 pb-2 rounded-t-lg border-b-8 border-yellow-500 transition-all duration-200 {$activeSection === 'particulieren' ? 'bg-yellow-400 text-black' : 'hover:bg-gray-800'}"
			...
			<button 
				class="flex items-center gap-2 px-4 py-2 ml-2 rounded-t-lg border-b-8 border-green-600 transition-all duration-200 {$activeSection === 'partners' ? 'bg-green-500 text-black' : 'hover:bg-gray-800'}"
