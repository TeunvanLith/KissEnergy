...
<div class="w-full bg-black">
	<div class="max-w-[1400px] mx-auto">
		...
		<div class="flex text-white pt-4 px-6">
			<button 
				class="flex items-center gap-2 px-4 py-2 pb-4 rounded-t-lg border-b-4 border-yellow-500 transition-all duration-200 {$activeSection === 'particulieren' ? 'bg-yellow-400 text-black font-bold' : 'hover:bg-gray-800'}"
			...
			<button 
				class="flex items-center gap-2 px-4 py-2 ml-2 rounded-t-lg border-b-4 border-green-600 transition-all duration-200 {$activeSection === 'partners' ? 'bg-green-500 text-black font-bold' : 'hover:bg-gray-800'}"
	...
	<nav class="max-w-[1400px] mx-auto py-4 px-6 flex flex-wrap items-center justify-left">
		...
		<div class="flex items-center mr-6">
			<a href="/" class="flex items-center">
				<img src="https://i.ibb.co/fYJgHQj6/Vertrouwd-dichtbij-logo.png" alt="Bedrijfslogo" class="h-10 w-auto" />
			</a>
		</div>
		...
		<div class="flex flex-wrap">
			{#each menus[$activeSection] as item, i}
				<a 
					href={item.path} 
					class="relative px-4 py-2 text-gray-800 font-medium hover:text-blue-600 transition-colors duration-200 {i !== 0 ? 'ml-2 md:ml-6' : ''}"
				>
					{item.name}
